...
class UserTest < ActiveSupport::TestCase
  ...
  test "name should be present" do
    @user.name = " "
    assert_not @user.valid? #ちゃんと上の状態がダメであると判断するかをテスト(app/models/user.rbの中で設定する)
  end
  ...
  test "email validation should accept valid address" do
    ...
    valid_address.each do |address|
      @user.email = address
      assert @user.valid?, "#{address.inspect} should be valid" #このエラーメッセージを付け加えることでどこで間違えたかがすぐわかる
    end
  end
  ...
  test "associated microposts should be destroyed" do
    @user.save
    @user.microposts.create!(create: "Lorem ipsum")
